#!/usr/bin/env python
"""
tessolena.py - work with hocr of tesseract instead of API for coordinates
- art rhyno, conifer/hackforge/ourdigitalworld

A sample for comparing the use of olena with newspaper images
with straight tesseract 
"""
...
OLENA_EXE = "/home/vmplanet/src/ocr/olena/olena-2.0/_build/scribo/src/contest/hdlac-2011/content_in_hdoc_hdlac"

#path to invoke tesseract
TESSERACT_EXE = "/usr/local/bin/tesseract"

#box scale - some images benefit from doubling for OCR
BOX_SCALE = 1
...
#namespace used by olena
OLENA_NAMESPACE = 'http://schema.primaresearch.org/PAGE/gts/pagecontent/2010-03-19'

#namespace used by xhtml (normally)
HTML_NAMESPACE = 'http://www.w3.org/1999/xhtml'

#simple criteria to identify words
IS_STOP  = re.compile('\s')

#could also do a more elaborate stop character
#IS_STOP  = re.compile('[\.|,|;|\s]')
...
#scribo_viewer uses base name for examining regions, so follow convention for PAGE results
...
if os.path.isfile(img_base + ".xml") and return_code >= 0:
       ...
       #if olena can't identify any regions, process the entire image
       if len(pg_squares) == 0:
